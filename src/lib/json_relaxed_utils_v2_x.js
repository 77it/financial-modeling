...
function quoteKeysNumbersAndDatesForRelaxedJSON(input, unquotedStringsMarker = '', formulaAdvancedParsing = false) {
  ...
  while (i < n) {
    ...
    // Structure & context
    if (c === '{') { contextStack = (contextStack << 1) | 1; inObjectKey = true; out.push(c); i++; continue; }
    if (c === '[') { contextStack = (contextStack << 1); inObjectKey = false; out.push(c); i++; continue; }
    ...
    if (c === '}' || c === ']') { contextStack >>= 1; inObjectKey = (contextStack & 1) === 1; out.push(c); i++; continue; }
    ...
    if (token != null) {
      ...
      if (needsMarkerHandling) {
        tokenTrimmed = token.trimEnd();
        trailingWS = tokenTrimmed.length < token.length ? token.slice(tokenTrimmed.length) : '';
      } else {
        tokenTrimmed = token;
        trailingWS = '';
      }
