...
-------------------------------------------------- ----------------------------- --------------------- --------------------------
parseJsonToLocalDate() benchmark: 1.000.000 loop             1.0 s           1.0 (949.6 ms …    1.1 s)    1.1 s    1.1 s    1.1 s
...
import { parseJsonToLocalDate, stripTimeToLocalDate } from '../../src/lib/date_utils.js';

const loopCount = 1_000_000;

Deno.bench(`parseJsonToLocalDate() benchmark: ${loopCount.toLocaleString('it-IT')} loop`, () => {
  // loop `loopCount` times
  for (let i = 0; i < loopCount; i++) {
    const parsed = parseJsonToLocalDate('2022-12-25').getTime();
  }
});
