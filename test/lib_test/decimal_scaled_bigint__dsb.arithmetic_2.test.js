...
t('test roundToAccounting HalfEven comparing it with Decimal.js', () => {
  Decimal.set({ rounding: Decimal.ROUND_HALF_EVEN });
  _TEST_ONLY__set(
    () => {
      // rounds on 5th decimal
      const inputs = [
        // up
        '0.00055',
        '0.00056',
      ...
      for (let i = 0; i < inputs.length; i++) {
        const sig20 = stringToBigIntScaled(inputs[i]);
        const got = roundToAccounting(sig20);

        const d = new Decimal(inputs[i]);
        const expSig20 = decSnapToAccountingSig20(d, Decimal.ROUND_HALF_EVEN);
        assert.strictEqual(got, expSig20);

        assert.strictEqual(bigIntScaledToString(got, { trim: true }), outputs[i]);
      }
    },
    { decimalScale: MATH_SCALE, accountingDecimalPlaces: ACCOUNTING_DECIMAL_PLACES, roundingMode: ROUNDING_MODES.HALF_EVEN });
});

t('test roundToAccounting HalfUp comparing it with Decimal.js', () => {
  Decimal.set({ rounding: Decimal.ROUND_HALF_UP });
