...
t('parseTextToLocalDate as Date - short date 2021-01-09', () => {
  const date = '2021-01-09';
  const parsedDate = parseTextToLocalDate(date);
  const expectedDate = new Date(2021, 0, 9);
  assert.equal(parsedDate.toISOString(), expectedDate.toISOString());
});

t('parseTextToLocalDate as Date - short date 2021-1-9 is valid', () => {
  const date = '2021-1-9';
  const parsedDate = parseTextToLocalDate(date);
  const expectedDate = new Date(2021, 0, 9);
  assert.equal(parsedDate.toISOString(), expectedDate.toISOString());
...
t('parseTextToLocalDate as Date - short date 2023/01/32: 32th day is parsed as the first day of the next month', () => {
...
t('parseTextToLocalDate as Date - parses a fully formed ISO date with Z', () => {
...
t('parseTextToLocalDate as Date - parses a fully formed ISO date with Z + spaces', () => {
  const date = '   2000-03-15T05:20:10.123Z   ';
  const parsedDate = parseTextToLocalDate(date);
  const expectedDate = new Date(2000, 2, 15, 5, 20, 10, 123);
  assert.equal(parsedDate.toISOString(), expectedDate.toISOString());
...
//#region parseTextToUTCDate as UTC #1
t('short date 2021-01-09, explicit UTC option', () => {
  const date = '2021-01-09';
  const parsedDate = parseTextToUTCDate(date);
  const expectedDate = '2021-01-09T00:00:00.000Z';
  assert.equal(parsedDate.toISOString(), expectedDate);
});
...
//#endregion parseTextToUTCDate as UTC #1
...
t('parses a fully formed ISO date with zero offset', () => {
  ...
  const expectedDate = '2000-03-15T05:20:10.000Z';
  const parsedDate = parseTextToUTCDate(zeroOffset);
  assert.equal(parsedDate.toISOString(), expectedDate);
});
...
t('returns an invalid date for anything else', () => {
  assert.equal(parseTextToUTCDate('').toString(), 'Invalid Date');
  assert.equal(parseTextToUTCDate('invalid').toString(), 'Invalid Date');
  assert.equal(parseTextToUTCDate('2020\\10\\10').toString(), 'Invalid Date');
});
//#region parseTextToUTCDate #2
