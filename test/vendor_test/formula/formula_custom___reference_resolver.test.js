...
import { reference_with_callCounter as reference, getCallCounter, resetCallCounter } from './_formula__reference_and_functions.js';
...
t('hapi formula with custom variable resolver', () => {
  ...
  const simple_formula = new Parser('a+b+1', { reference }).toFunction();
  assert.deepStrictEqual(getCallCounter(), 0); // right, no calls yet
  assert.deepStrictEqual(simple_formula(), convertWhenFmlEvalRequiresIt(6));
  assert.deepStrictEqual(getCallCounter(), 2); // two calls during evaluation
  assert.deepStrictEqual(simple_formula(), convertWhenFmlEvalRequiresIt(8));
  assert.deepStrictEqual(getCallCounter(), 4); // right, +2 calls during evaluation
  assert.deepStrictEqual(simple_formula(), convertWhenFmlEvalRequiresIt(10));
  assert.deepStrictEqual(getCallCounter(), 6); // right, +2 calls during evaluation

  const string_to_parse_1 = 'a + 1.99 + a.b + (((a+1)*2)+1) + a + $.ciao + $ + ((ad$.ciao+10)-2*5)';
  const expected_1 = convertWhenFmlEvalRequiresIt('54.99');
  const formula1 = new Parser(string_to_parse_1, { reference }).toFunction();
  assert.deepStrictEqual(formula1(), expected_1);
  assert.deepStrictEqual(getCallCounter(), 13); // right, +7 calls during evaluation
  const expected_2 = convertWhenFmlEvalRequiresIt('82.99');
  assert.deepStrictEqual(formula1(), expected_2);
  assert.deepStrictEqual(getCallCounter(), 20); // right, +7 calls during evaluation

  // X is undefined
  assert.throws(() => {
    const formula2 = new Parser('a + x + 1', { reference }).toFunction();
    formula2();
  });

  // AAA_from_context is defined in context, passing a custom reference resolver that reads from context too, works
  const formula3 = new Parser('a + AAA_from_context + 1', { reference }).toFunction();
  assert.deepStrictEqual(formula3({AAA_from_context: 90}), convertWhenFmlEvalRequiresIt(115));  // a = 1 + getCallCounter() => 7 at this point
