...
t('formula calling directly YAML, outside functions', () => {
  const expected = {a: convertWhenFmlEvalRequiresIt(11), b: "mam ma", "a + 7 c": null, d: new Date(2025, 11, 31)};
...
t('formula with YAML, array, date, etc', () => {
  assert.deepStrictEqual(new Parser('Q( {a: [1, 2025/12/31, abcd e, q(10) + 1 * 1 + 9*0] } )', { functions }).evaluate().a, [1, new Date(2025, 11, 31), "abcd e", convertWhenFmlEvalRequiresIt(11)]);
