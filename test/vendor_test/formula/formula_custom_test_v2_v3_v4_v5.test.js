...
import { toStringWhenNeeded } from './_formula__tests_settings.js'
...
t('formula calling directly YAML, outside functions', () => {
  const expected = {a: toStringWhenNeeded(11), b: "mam ma", "a + 7 c": null, d: new Date(2025, 11, 31)};
...
t('formula with YAML, array, date, etc', () => {
  assert.deepStrictEqual(new Parser('Q( {a: [1, 2025/12/31, abcd e, q(10) + 1 * 1 + 9*0] } )', { functions }).evaluate().a, [1, new Date(2025, 11, 31), "abcd e", toStringWhenNeeded(11)]);
  ...
  assert.deepStrictEqual(new Parser('q( 2025-12-31 )', { functions }).evaluate(), toStringWhenNeeded(2025 - 12 - 31));
...
t('arithmetics with negative numbers  // only v4 supports numbers with explicit + sign', () => {
  assert.deepStrictEqual(new Parser('-10 / +2').evaluate(), toStringWhenNeeded(-5));
  assert.deepStrictEqual(new Parser('-10 * +2').evaluate(), toStringWhenNeeded(-20));
});
