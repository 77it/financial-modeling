...
Notes:
  - SOBOL uses a single sampler advanced across blocks to preserve low-discrepancy.
    ...
    if engine == "SOBOL":
        ...
        if exact_n and U.shape[0] > n_requested:
            ...
            if U.shape[0] != n_requested:
                print(f"[INFO] SOBOL blocks: generated {U.shape[0]} (ceil to full blocks) for requested n={n_requested}.")
...
def validate_and_prepare(
    ...
    # Correlation matrix (robust like v5: use first column as labels; no strict header)
...
def main():
    ...
    # Settings summary (interpolated values; ASCII)
    ...
    if engine == "SOBOL":
        ...
        if engine == "SOBOL" and not args.exact_n:
            print(f"[TIP] Padded to a multiple of {args.block_size} due to SOBOL blocks. Use --exact-n to cut to exactly {runs} or change --block-size.")
