...
def section_records(
    ...
    try:
        df = wb.parse(sec.sheet_name, header=None, dtype=object, engine="openpyxl")
    ...
    for kpi, label_text in sec.labels_map.items():
        ...
        for c0, iso_date in date_cols:
            ...
            if val is None:
                # Skip empty/non-numeric cells silently but keep a breadcrumb:
                # Uncomment next line if you want explicit warnings per-miss:
                # warn(f"[{sec.name}] Empty/non-numeric value for KPI '{kpi}' at date {iso_date}.")
                continue
...
def extract_from_workbook(
    ...
    try:
        wb = pd.ExcelFile(xlsx_path, engine="openpyxl")
    except Exception as e:
