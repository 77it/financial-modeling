...
def validate_and_prepare(
    ...
    if not discrete.empty:
        ...
        if discrete["value"].isna().any():
            bad_examples = discrete[discrete["value"].isna()][["variable", "month"]].head(10).values.tolist()
    ...
    if not bad_nan.empty:
        examples = bad_nan[["variable", "month"]].head(10).values.tolist()
    ...
    if mask_swap.any():
        ex = tri_rows.loc[mask_swap, ["variable", "month", "min", "max"]].head(10).values.tolist()
    ...
    asym_mask = np.triu(np.abs(diff) > 1e-10, k=1)
    if np.any(asym_mask):
    ...
    if clip_delta > 1e-12:
        num_clipped = int(np.sum(np.abs(before_clip - corr_matrix) > 1e-12))
