...
def read_inputs(xlsx_path: str):
    ...
    must = {"DRV", "DRV-triangular", "Correlation", "Settings"}
    have = set(xls.sheet_names)
    missing = must - have
    if missing:
        raise SystemExit(f"[ERROR] Missing sheets: {sorted(missing)}")

    variables = pd.read_excel(xls, "DRV").fillna("")

    # Read triangular/uniform params from DRV-triangular; map best/worst to min/max.
    params_raw = pd.read_excel(xls, "DRV-triangular").fillna("")
    # Normalize headers (case/space-insensitive)
    params = params_raw.rename(
        columns={c: str(c).strip().lower() for c in params_raw.columns}
    )
    # Accept synonyms and map to canonical names
    col_map = {}
    if "best" in params.columns:   col_map["best"]  = "min"
    if "worst" in params.columns:  col_map["worst"] = "max"
    if "mode" in params.columns:   col_map["mode"]  = "mode"
    if "month" in params.columns:  col_map["month"] = "month"
    if "variable" in params.columns: col_map["variable"] = "variable"
    params = params.rename(columns=col_map)

    # Keep only the columns we use; silently ignore 'step' or any extras
    keep = [c for c in ["variable", "month", "min", "mode", "max"] if c in params.columns]
    params = params[keep].copy()

    # Discrete optional
    discrete = (
        pd.read_excel(xls, "DRV-discrete").fillna("")
        if "DRV-discrete" in have else
        pd.DataFrame(columns=["variable", "month", "value", "prob"])
    )
...
def validate_and_prepare(
    ...
    if variables is None or variables.empty:
        raise SystemExit("[ERROR] 'DRV' sheet is empty.")

    # Required cols
    for col in ["variable", "dist", "ar1_enabled"]:
        if col not in variables.columns:
            raise SystemExit(f"[ERROR] DRV sheet missing column '{col}'")
    ...
    for col in ["variable", "month"]:
        if col not in params.columns:
            raise SystemExit(f"[ERROR] DRV-triangular sheet missing column '{col}'")
    # ensure min/mode/max exist (mode can be absent and will be added as NaN)
    if "min" not in params.columns and "best" in params.columns:
        params = params.rename(columns={"best": "min"})
    if "max" not in params.columns and "worst" in params.columns:
        params = params.rename(columns={"worst": "max"})
    ...
    if not discrete.empty:
        for col in ["variable", "month", "value", "prob"]:
            if col not in discrete.columns:
                raise SystemExit(f"[ERROR] DRV-discrete sheet missing column '{col}'")
    ...
    if not bad_nan.empty:
        ...
        raise SystemExit(f"[ERROR] DRV-triangular best/worst contain non-numeric values. Examples: {examples} ...")
    ...
    if mask_swap.any():
        ...
        print(f"[TIP] Swapped best/worst for {mask_swap.sum()} row(s) where best>worst. Examples (variable, month, best_before, worst_before): {ex} ...")
    ...
    for _, r in tri_rows.iterrows():
        ...
        if not np.isfinite(md_in):
            ...
            fix = "mode slightly < best; clamped"
            md_out = lo
        elif md_in > hi:
            fix = "mode slightly > worst; clamped"
            md_out = hi
        else:
            md_out = md_in
            fix = "ok"
    ...
    try:
        ...
        raise SystemExit("[ERROR] Correlation sheet must have rows/cols for all variables in 'DRV'.")
...
def generate_draws(
    ...
    if burnin and burnin > 0:
        ...
        if (engine or DEFAULT_ENGINE).upper() == "SOBOL":
