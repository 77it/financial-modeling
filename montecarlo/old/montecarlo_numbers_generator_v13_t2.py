...
def validate_and_prepare(
    ...
    if not discrete.empty:
        ...
        discrete[COL_DISC_VALUE] = pd.to_numeric(discrete[COL_DISC_VALUE], errors="coerce")
        if discrete[COL_DISC_VALUE].isna().any():
            bad_examples = discrete[discrete[COL_DISC_VALUE].isna()][[COL_VARIABLE, COL_MONTH]].head(TIP_SAMPLE_COUNT).values.tolist()
            raise SystemExit(f"[ERROR] Discrete.value contains non-numeric values. Examples: {bad_examples} ...")
        discrete[COL_DISC_PROB] = pd.to_numeric(discrete[COL_DISC_PROB], errors="coerce")
        if discrete[COL_DISC_PROB].isna().any():
            raise SystemExit("[ERROR] Discrete.prob contains non-numeric values.")
        # per (variable, month) must sum to 1
        sums = discrete.groupby([COL_VARIABLE, COL_MONTH])[COL_DISC_PROB].sum().round(12)
    ...
    # Auto-swap where best > worst, with a concise TIP
    mask_swap = tri_rows[COL_BEST] > tri_rows[COL_WORST]
    if mask_swap.any():
        ex = tri_rows.loc[mask_swap, [COL_VARIABLE, COL_MONTH, COL_BEST, COL_WORST]].head(TIP_SAMPLE_COUNT).values.tolist()
    ...
    if not discrete.empty:
        for (v, p), g in discrete.groupby([COL_VARIABLE, COL_MONTH]):
...
def generate_draws(
    ...
    out_dict = {"run": out_run, COL_VARIABLE: out_var}
