import { isValidDate, parseJsonDate, differenceInCalendarDays as differenceInCalendarDays_lib, differenceInUTCCalendarDays, excelSerialDateToUTCDate, excelSerialDateToDate } from '../../src/lib/date_utils.js';
...
_describe('parseJsonDate as Date', () => {
...
_describe('parseJsonDate as UTC #1', () => {
    it('short date 2021-01-09, explicit UTC option', () => {
        const date = '2021-01-09';
        const parsedDate = parseJsonDate(date, {asUTC: true});
    ...
    it('short date 2021-01-09', () => {
        const date = '2021-01-09';
        const parsedDate = parseJsonDate(date);
        const expectedDate = '2021-01-09T00:00:00.000Z';
        assert.equal(parsedDate.toISOString(), expectedDate);
    })
...
_describe('parseJsonDate as UTC #2', () => {
    ...
    it('parses a fully formed ISO date with zero offset', () => {
        ...
        const expectedDate = '2000-03-15T05:20:10.000Z'
        const parsedDate = parseJsonDate(zeroOffset)
        assert.equal(parsedDate.toISOString(), expectedDate)
    })
    ...
    it('returns an invalid date for anything else', () => {
        assert.equal(parseJsonDate('').toString(), 'Invalid Date')
        assert.equal(parseJsonDate('invalid').toString(), 'Invalid Date')
        assert.equal(parseJsonDate('2020\\10\\10').toString(), 'Invalid Date')
    })
})
